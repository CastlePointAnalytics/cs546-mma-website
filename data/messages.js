...
module.exports = {
	async getMessage(id) {
		...
		if (!id || typeof id != 'string' || id.length < 1)
			throw 'Could not get id for message!';
	...
	async getAllMessagesFromBout(boutcard_Id) {
		...
		if (
			!boutcard_Id ||
			typeof boutcard_Id != 'string' ||
			boutcard_Id.length < 1
		)
			throw 'Bout ID passed was invalid type';

		const collection = await messages();

		const messageList = await collection
			.find({ boutcard_Id: boutcard_Id })
			.toArray();
		if (messageList.length === 0)
			throw 'Could not find messages for provided fight!';
	...
	async createMessage(boutcard_Id, text, timestamp, user_id, username) {
		...
		if (
			...
			throw 'Bout ID passed was invalid type'; // would be an internal server error, not user
		text = text.trim();
		if (!text || typeof text != 'string' || text.length < 1)
			throw 'Please input text!';
		// user_id = user_id.trim();

		if (!user_id) throw 'User ID passed was invalid type';

		// if (!user_id || typeof user_id != 'string' || user_id.length < 1)
		// 	throw 'User ID passed was invalid type'; // would be an internal server error, not user
		// parent = parent.trim();
		...
		if (!timestamp || typeof timestamp.toUTCString() != 'string')
			throw 'Timestamp error'; // Server error, not user

		username = username.trim();
		if (!username || typeof username != 'string' || username.length < 1)
			throw 'Username pass was invalid type'; // would be an internal server error, not user
	...
	async updateMessage(id, text, user_id, timestamp) {
		...
		user_id = user_id.trim();
		if (!user_id || typeof user_id != 'string' || user_id.length < 1)
			throw 'User ID passed was invalid type';

		if (message.user_id != user_id)
			throw 'Request came from unauthorized user!';
